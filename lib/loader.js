...
async function resolveURI(uri, ipfs = null) {
...
export async function loadURI(uri, ipfs = null) {
...
export async function loadText(shex, ipfs = null) {
    const merged = Util.emptySchema();
    const loaded = new Set([]);
    async function load(schema) {
        Util.merge(merged, schema, false, true);
        if (Array.isArray(schema.imports)) {
            await Promise.all(schema.imports
